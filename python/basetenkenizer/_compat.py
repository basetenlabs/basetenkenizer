...
def _reject_unsupported_kwargs(kwargs: dict) -> None:
    if kwargs:
        ...
        raise TypeError(f"unsupported basetenkenizer encode option(s): {names}")
...
class _TokenizerShim:
    ...
    def encode(
        ...
        **kwargs,
        ...
        if pair is not None:
            raise NotImplementedError("pair encoding is not supported by basetenkenizer")
        if is_pretokenized:
            raise NotImplementedError(
                "pre-tokenized input is not supported by basetenkenizer"
            )
    ...
    def encode_batch(
        ...
        **kwargs,
        ...
        if is_pretokenized or any(isinstance(inp, (list, tuple)) for inp in inputs):
            raise NotImplementedError(
                "pair/pre-tokenized batch encoding is not supported by basetenkenizer"
            )
        if split_special_tokens is None:
            split_special_tokens = self._encode_special_tokens
