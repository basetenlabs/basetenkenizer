#!/usr/bin/env python3
"""
Benchmark basetenkenizer vs stock tokenizer via Dynamo (frontend + SGLang backend).

Launches a Dynamo frontend and SGLang backend with and without basetenkenizer env
...
def _launch_dynamo(
    ...
    *,
    ...
    if patched:
        frontend_cmd.extend(["--tokenizer", "basetenkenizer"])
...
def _download_dataset(name: str) -> list[dict]:
    ...
    cache = os.path.join(tempfile.gettempdir(), "basetenkenizer_bench_cache")
...
def _print_comparison(
    ...
    title: str = "Dynamo Benchmark: baseline vs basetenkenizer",
) -> None:
    print()
    print("=" * _W)
    print(f"  {title}")
    print(f"  Model: {model}")
    print("=" * _W)
    print(f"  {'Metric':<32} {'Baseline':>12} {'Basetenkenizer':>12} {'Change':>10}")
    print("-" * _W)
...
def _print_bucket_comparison(
    ...
    print(f"  Model: {model}")
    print("=" * row_w)
    print(f"  {'Bucket':<{label_w}} {'N':>6} {'Baseline P50':>14} {'Basetenkenizer P50':>14} {'Change':>10}")
    print("-" * row_w)
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Benchmark basetenkenizer vs stock tokenizer via Dynamo "
    ...
    group.add_argument(
        ...
        help="Only run the patched (basetenkenizer) benchmark",
    )
    ...
    parser.add_argument(
        ...
        help="Compare tokenization speed only (stock vs basetenkenizer), "
    ...
    if args.tokenize_only:
        ...
        if not args.patched_only:
            ...
            baseline_metrics, baseline_latencies = _run_tokenize_bench(
                args.model, bench_prompts, use_basetenkenizer=False,
                batch_size=args.batch_size,
            )
            _print_run_summary("BASELINE", baseline_metrics)

        if not args.baseline_only:
            print(f"\n  [BASETENKENIZER] Tokenizing {len(bench_prompts)} prompts (basetenkenizer)...")
            patched_metrics, patched_latencies = _run_tokenize_bench(
                args.model, bench_prompts, use_basetenkenizer=True,
                batch_size=args.batch_size,
            )
            _print_run_summary("BASETENKENIZER", patched_metrics)

        if baseline_metrics and patched_metrics:
            _print_comparison(
                args.model, baseline_metrics, patched_metrics,
                title="Tokenization Benchmark: stock vs basetenkenizer",
            )
        ...
        if args.output:
            ...
            if patched_metrics:
                results["basetenkenizer"] = patched_metrics
    ...
    def _run_one(*, patched: bool, port: int, external_url: str | None = None) -> tuple[dict[str, float], list[float]]:
        tag = "BASETENKENIZER" if patched else "BASELINE"
