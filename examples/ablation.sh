...
echo "model,dataset,batch_size,input_index,input_char_len,output_token_len,hf_duration_ms,basetenkenizer_duration_ms" \
    > "$COMBINED_CSV"
    ...
    for dataset in "${DATASETS[@]}"; do
        for bs in "${BATCH_SIZES[@]}"; do
            ...
            if [[ -f "$log_file" ]]; then
                ...
                ft_total=$(awk '/basetenkenizer total:/{print $(NF-1)}' "$log_file" 2>/dev/null)
                speedup=$(awk '/Speedup:/{gsub(/x/,"",$NF); print $NF}' "$log_file" 2>/dev/null)
                hf_total="${hf_total:--}"
                ft_total="${ft_total:--}"
                speedup="${speedup:--}"
