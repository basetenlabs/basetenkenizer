...
fn main() -> Result<()> {
    ...
    let tokenizer =
        basetenkenizer::Tokenizer::from_model(model_name).context("failed to load tokenizer")?;
    ...
    use basetenkenizer::pre_tokenized::{PreTokenizedString, Split as PtSplit};
    ...
    let split =
        basetenkenizer::Split::from_config(&json!({"Regex": split_pattern}), "Isolated", false)
            .unwrap();

    // Create ByteLevel pre-tokenizer directly
    let byte_level = basetenkenizer::ByteLevel::from_config(false, true, false).unwrap();
